...
(define-extended-language Ev L #;L+Γ
  (E (e E) #;(v E)
     (E e)
     (mkVarT E)
     (apply e E)
...
(define red
  (reduction-relation
   ...
   (--> (in-hole E (apply (mkVarT (λ (x_0 t) -> e_0)) e_1))
        (in-hole E e_1)
        (side-condition (equal? (term x_0) (term e_0)))
        "v1")

   (--> (in-hole E (apply (mkVarT (λ (x_0 t) -> e_0)) e_1))
        (in-hole E (mkVarT e_1))
        (side-condition (and (symbol? (term e_0))
                             (not (equal? (term x_0) (term e_0)))))
        "v2")
   
   (--> (in-hole E (apply (apply (mkVarT (λ (x_1 t_1) (x_2 t_2) -> e_0)) e_1) e_2))
        (in-hole E e_1)
        (side-condition (equal? (term x_1) (term e_0)))
        "v1-2a")
   
   (--> (in-hole E (apply (apply (mkVarT (λ (x_1 t_1) (x_2 t_2) -> e_0)) e_1) e_2))
        (in-hole E e_2)
        (side-condition (equal? (term x_2) (term e_0)))
        "v1-2b")
   
   
   (--> (in-hole E (apply (apply (mkVarT (λ (x_1 t_1) (x_2 t_2) -> e_1)) e_2) e_3))
        (in-hole E (mkVarT e_1))
        (side-condition (and (symbol? (term e_1))
                             (not (or (equal? (term x_1) (term e_1))
                                      (equal? (term x_2) (term e_1))))))
        "v2-2")
...
(traces red
        (term (apply (apply (mkVarT (λ (a Int) (b Int) -> ((+ (foo a)) (bar b)))) va) vb)))
